...
# +++++++++++++++++++++++++++++++++++++++++++++++++=
  def farm
    puts "inside session farm"
    gold = rand(10..20)
    session[:activities] << "Earned #{gold} golds! ...#{Time.now.strftime('%Y/%m/%d %l:%M %P')})"
    session[:gold] += gold
    redirect_to :root
  end
# +++++++++++++++++++++++++++++++++++++++++++++=
...
# +++++++++++++++++++++++++++++++++++++++++++++=
  ...
  def casino
    ...
    if gold > 0
      ...
      session[:activities] << "Entered a casino and lost #{gold} golds... Ouch. (#{Time.now.strftime('%Y/%m/%d %l:%M %P')})"
    end
    session[:gold] += gold
